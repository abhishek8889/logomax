...
window.Echo.channel('message'+user_id)
    .listen('.messages',(e)=>{
        ...
        if(e.eventData.message != null){
           let message_html = '<div class="lv-chat"><p class="b-text">'+e.eventData.message+'</p></div>';
        }else{
           let message_html = '';
        }
       let files_html = [];
        if(files.length != 0){
            $.each(files ,function(key,value){
                let file_html = '<div class="lv-chat"><button>'+value+'</button></div>';
                files_html.push(file_html);
            });
        }
               
       let html = '<div class="sender-d"><div class="s-ryt-img"><span class="str"><img src="'+base_url+'/logomax_pages/img/star.svg"  class="img-fluid" alt="..."></span><span><img src="'+base_url+'/logomax_pages/img/h-dot.svg"  class="img-fluid" alt="..."></span></div><div class="lp-info d-flex"><div class="lp-img"><img src="'+base_url+'/logomax_pages/img/jord.png"  class="img-fluid" alt="..."></div><div class="lp-text"><h6>'+e.eventData.userdata.first_name+' '+e.eventData.userdata.last_name+' <span>10:00 am</span></h6> '+message_html+' '+files_html+' </div></div></div>';
