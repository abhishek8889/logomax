...
<section class="blog-sec ">
  <div class="container">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('/',app()->getLocale()) }}">Home</a></li>
    ...
    <div class="blog-img-box">
      <div class="row">
        @if(isset($blogs[0])) 
          <?php  $blog = $blogs[0]; ?>
          <div class="col-md-8 ">
            <a class="blog_box d-block h-100" href="{{ url(app()->getLocale().'/blogs-details/' . ($blog->slug ?? '')) }}">
              <div class="blog-img">
                <img src="{{ asset('blog_images') }}/{{ $blog->banner_img ?? '' }}" alt="">
                <div class="content">
                  <div class="blog-dummy-text">
                    <h2>{{ $blog->translation->title ?? $blog->title }}</h2>
        ...
        @if(isset($blogs[1]) || isset($blogs[2]))
          <div class="col-md-4">
            @if(isset($blogs[1]))
            <?php $blog = $blogs[1]; ?>
              <a class="blog_box d-block" href="{{ url(app()->getLocale().'/blogs-details/' . ($blog->slug ?? '')) }}">
                <div class="group-img-{{ $key ?? '1' }}">
                  <img src="{{ asset('blog_images') }}/{{ $blog->banner_img ?? '' }}" alt="">
                  <div class="passage-text">
                    <p>{{ $blog->translation->title ?? $blog->title }}</p>
                  </div>
                </div>
              </a>
            @endif
        ...
        <div class="col-lg-4 col-md-6 {{ $k >= 9 ? 'blogs-data d-none' : '' }}">
          <div class="blog-content">
            <div class="recent-blog-img">
              <a href="{{ url(app()->getLocale().'/blogs-details/' . ($blog->slug ?? '')) }}">
                <img src="{{ asset('blog_images') }}/{{ $blog->banner_img ?? '' }}" alt="">
              </a>
            </div>
            <div class="recent-text">
              ...
              <div class="simply-text">
                <a href="{{ url(app()->getLocale().'/blogs-details/'.($blog->slug ?? '')) }}">
                  <h6>{{ $blog->translation->title ?? $blog->title }}</h6>
                </a>
                <p>{{ $blog->translation->sub_title ?? $blog->sub_title }}</p>
              </div>
            </div>
          </div>
        </div>
