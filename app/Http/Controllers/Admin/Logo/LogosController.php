...
class LogosController extends Controller
{
    ...
    public function updateStatus(Request $request){
        ...
        if($request->action == 'approved'){
            if($request->logo_type == 'low-price'){
                $price = 199;
            }elseif($request->logo_type == 'premium'){
                $price = 299;
            }
            $logos = Logo::find($request->id);
            $logos->approved_status = 1;
            $logos->price_for_customer = $price;
            $logos->price_for_designer = $price;
            ...
            return redirect()->back()->with('success','You have disapproved this logo successfully.');
        }
    }
    public function logoFacilities(Request $req){
        $admin_selected_lang = $req->session()->get('on_admin_lang_code');
        
        $logoFacilities  = LogoFacilities::where('lang_code',$admin_selected_lang)->get();
    ...
    public function logoFacilitiesAdd(Request $req){
        ...
        if(!empty($id)){
            ...
            if(!empty($facility)){
                ...
                $facility->facilities_name	 = $req->name;
                $facility->description = $req->description;
                $facility->language = $admin_selected_lang;
                $facility->lang_code = $admin_selected_lang;
                $facility->status = 1;
                $facility->save();
