...
class GoogleController extends Controller
{
    ...
    try {
        ...
        if($user){ 
            $data = array(
                'email' => $user->email,
                'password' => $metadata->id,
            );
            if (Auth::attempt($data)) {
                if (Auth::user()->email_verified == 1) {
                    switch (Auth::user()->role_id) {
                        case 1:
                            return redirect('/')->with('success', 'Welcome ' . Auth::user()->name . ' to home page');
                        case 2:
            ...
            $user = User::where('email',$metadata->user['email'])->first();
            if($user){
            ...
            $user = new User;
            $user->first_name = $metadata->user['name'];
    ...
    public function handleGoogleCallback(){
       try {
        ...
        if($user){
            ...
            $user = new User;
            $user->first_name = $googledata->name;
